...
import { Shield, Mail, Lock, ArrowRight, AlertCircle, Eye, EyeOff, Sparkles } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

export const Visme3DAuth: React.FC = () => {
  ...
  useEffect(() => {
    // Inject Visme 3D Form Engine Script dynamically
  ...
  return (
    <div className="relative min-h-[calc(100vh-64px)] bg-[#0D1117] flex flex-col items-center justify-center px-6 py-12 text-[#F0F6FC] font-sans">
      {/* Background Ambient Glow */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[500px] bg-[radial-gradient(ellipse_at_center,_rgba(126,231,135,0.18)_0%,_transparent_70%)] blur-[100px]" />
      </div>

      <div className="relative z-10 w-full max-w-5xl grid lg:grid-cols-2 gap-8 items-center">
        {/* LEFT COLUMN: 3D Visme Character Embed Frame */}
        <motion.div
          initial={{ opacity: 0, x: -30 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.6 }}
          className="relative bg-[#161B22]/80 backdrop-blur-xl border border-[#30363D] rounded-2xl p-6 shadow-2xl overflow-hidden min-h-[480px] flex flex-col justify-center items-center"
        >
          <div className="absolute top-4 left-4 inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#7EE787]/15 border border-[#7EE787]/30 text-[11px] font-mono text-[#7EE787]">
            <Sparkles className="w-3.5 h-3.5" />
            <span>3D INTERACTIVE AVATAR</span>
          </div>

          {/* Visme 3D Container Embed */}
          <div
            className="visme_d w-full min-h-[420px] rounded-xl overflow-hidden"
        ...
        {/* RIGHT COLUMN: Fast & Secure JWT Authentication Form */}
        <motion.div
          initial={{ opacity: 0, x: 30 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.6 }}
        ...
        >
          ...
          <div className="relative bg-[#161B22]/90 backdrop-blur-xl border border-[#30363D] rounded-2xl p-8 shadow-2xl">
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <h2 className="font-heading font-bold text-2xl text-[#F0F6FC]">
                DevSec Agent
              </h2>
              <p className="font-mono text-xs text-[#8B949E] mt-1">
                Fast & Secure JWT Authentication
              </p>
            </div>
